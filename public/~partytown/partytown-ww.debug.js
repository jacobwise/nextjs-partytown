(self => {
    ...
    class WorkerScriptElement extends WorkerSrcElement {
        ...
        set src(url) {
            if (this[WinIdKey] === webWorkerCtx.$winId$) {
                url = resolveUrl(url) + "";
                setInstanceStateValue(this, 3, url);
                this[ImmediateSettersKey] && this[ImmediateSettersKey].push([ [ "src" ], serializeForMain(this[WinIdKey], this[InstanceIdKey], url) ]);
            } else {
                setter(this, [ "src" ], url);
            }
        }
    ...
    const queueTask = (instance, $accessType$, $memberPath$, $data$, $immediateSetters$, $newInstanceId$) => {
        ...
        return drainQueue(winId, accessReqTask.$instanceId$, $memberPath$, $forwardToWorkerAccess$, winQueue);
    };
    ...
    const setter = (instance, memberPath, value) => {
        ...
        if (instance[ImmediateSettersKey]) {
            instance[ImmediateSettersKey].push([ memberPath, serializeForMain(instance[WinIdKey], instance[InstanceIdKey], value) ]);
        } else {
    ...
    const callMethod = (instance, memberPath, args, immediateSetters, newInstanceId) => {
        applyBeforeSyncSetters(instance[WinIdKey], instance);
        const rtn = queueTask(instance, 2, memberPath, serializeForMain(instance[WinIdKey], instance[InstanceIdKey], args), immediateSetters, newInstanceId);
    ...
    const deserializeFromMain = (winId, instanceId, memberPath, serializedValueTransfer, serializedType, serializedValue, obj, key) => {
        if (serializedValueTransfer) {
            ...
            if (0 === serializedType) {
                return serializedValue.map((v => deserializeFromMain(winId, instanceId, memberPath, v)));
            }
            if (3 === serializedType) {
                obj = {};
    ...
    const deserializeRefFromMain = (instanceId, memberPath, {$winId$: $winId$, $refId$: $refId$}) => {
        ...
        workerRefMap || setStateValue(instanceId, 0, workerRefMap = {});
        workerRefHandler = workerRefMap[$refId$];
        workerRefHandler || (workerRefMap[$refId$] = workerRefHandler = function(...args) {
            ...
            3 === msgType ? (initScript => {
                ...
                for (const accessReqTask of accessReq.$tasks$) {
                    ...
                    try {
                        ...
                        0 === accessType ? rtnValue = instance[lastMemberName] : 1 === accessType ? instance[lastMemberName] = data : 2 === accessType && (rtnValue = 4 === instanceId && "createElement" === lastMemberName ? callMethod(instance, memberPath, data, accessReqTask.$immediateSetters$, accessReqTask.$newInstanceId$) : instance[lastMemberName].apply(instance, data));
